...
def print_stats(test_loss, test_acc, test_precision, test_recall, model_name):
    ...
    data_df = pd.read_pickle(path.join('polarity_3_classes', path.join('results', 'data_df.pkl')))
    ...
    if test_acc > best_acc:
        ...
        model.save(path.join('polarity_3_classes', path.join('tf_models', model_name+"_model")))
        
    data_df.to_pickle(path.join('polarity_3_classes', path.join('results', 'data_df.pkl')))
...
def build_model(word_index, filters, kernel_array):
    ...
    outputs = tf.keras.layers.Dense(3, activation='softmax')(merged_inputs)
...
print(pd.read_pickle(path.join('polarity_3_classes', path.join('results', 'data_df.pkl'))))
