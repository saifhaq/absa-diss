...
data_df = pd.read_pickle(path.join('inputs', path.join('results', 'stoplist_tokenizer.pkl')))

with_stoplist = data_df[data_df.stoplist == True].f1.to_list()
no_stoplist = data_df[data_df.stoplist == False].f1.to_list()

stoplist_with_oov = [ 0.385142, 0.414122,  0.417132, 0.413858, 0.422932, 0.423041, 0.426443, 0.423221,0.433272,0.421348,0.420358,0.421642]
...
plt.plot(n_words, with_stoplist, color='g', label="Stopwords removed")
plt.plot(n_words, stoplist_with_oov, color='blue', label="Stopwords replaced with <oov>")
