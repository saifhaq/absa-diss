...
def load_data(n_classes, n_words, stop_words = 1):
    ...
    :stopwords: Choice of (1, 2, 3) with
        - 1 being no stoplist
        - 2 being remove stopwords
        - 3 being replace stopwords with <oov> 
    """
    ...
    if stop_words == 2: 
        ...
        stopwords = stoplist()
        train_df = apply_stoplist(train_df)
        test_df = apply_stoplist(test_df)
...
earlystop_callback = tf.keras.callbacks.EarlyStopping(monitor='val_acc', patience=5, restore_best_weights=True)  
n_words_array = [250, 500, 750, 1000, 1250, 1500, 1750, 2000, 2250, 2500, 2750, 3000]


### Generate results for stopwords 3: stopwords replaced with oov token 
stoplist_choice = 1
...
### Generate results for stopwords 2: stopwords removed
stoplist_choice = 2
data_df = pd.DataFrame(columns = ['n_words', 'stoplist', 'f1'])
data_df_name = 'tokenizer_words_'+str(stoplist_choice)+'.pkl'
