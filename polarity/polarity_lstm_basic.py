...
def build_model(word_index, filters, kernel_array):
    ...
    model = tf.keras.Model(input_layer, outputs = outputs)
...
for i in range(0,1):
    ...
    test_loss, test_acc, test_precision, test_recall = model.evaluate(x_tests[0], y_test)
    test_f1 = print_stats(test_loss, test_acc, test_precision, test_recall, 'lstm_basic')
