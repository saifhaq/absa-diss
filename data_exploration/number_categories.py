...
n = 16
data_df = pd.DataFrame(columns = ["Aspect Category", "Training Samples", "Percentage"])
...
for i in range(0, n):
    ...
    data_df = data_df.append({'Aspect Category': DESIRED_CATEGORY, 'Training Samples': TRAIN_COUNT}, ignore_index=True)

n_sentences = len(sentences)

for index, row in data_df.iterrows():
    p = (row['Training Samples'] / n_sentences) *100
