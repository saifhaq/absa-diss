...
for i in range(0, len(n_words_array)):
    ...
    data_df = data_df.append({'n_words': int(n_words_array[i]), 'stoplist': True, 'f1': 0}, ignore_index=True)
    for j in range(1,6):
        x_train, y_train, x_val, y_val, x_test, y_test, word_index = load_data(16, n_words_array[i], True)
...
for i in range(len(n_words_array), 2*len(n_words_array)):
    
    data_df = data_df.append({'n_words': int(n_words_array[i-len(n_words_array)]), 'stoplist': False, 'f1': 0}, ignore_index=True)
    for j in range(1,6):
        x_train, y_train, x_val, y_val, x_test, y_test, word_index = load_data(16, n_words_array[i-len(n_words_array)], False)
...
print(data_df)
data_df.to_pickle(path.join('main_system', path.join('aspect', 'aspect_baselinenn_300glove_data')))
