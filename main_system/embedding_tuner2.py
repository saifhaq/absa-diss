...
def build_model(embedding_dimension, word_index):


    model = tf.keras.Sequential()
    model.add(tf.keras.layers.Embedding(len(word_index)+1, embedding_dimension))
...
print(data_df)
data_df.to_pickle(path.join('main_system', path.join('aspect', 'aspect_embedding_layer.pkl')))
